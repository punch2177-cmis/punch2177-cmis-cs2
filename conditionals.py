#My script will determine how you will die, whether it is in the near furture or 30 years away. I will take in random information about the player and ask them to make desicions. There desicions will consequently tell them about how they die. First, they will be asked how old they are and what their favorite number is to determine how long they have left to live. The second question asks about their favorite vacation spot in order to help determine how they will die in the next function. The second question is to determine how many enemies they have. The vacation spot and the number of enemies will determine whether the "calculation" function will return True or False. The next questions asks what they like to do on the weekends and how many friends they think they have. The value from the two functions will be used in the "calculation2" function and it will either return true or false. If true is returned in both the "calculation" and the "calculation2" function, then they will die a peaceful death. If, however, both of the fuctions return false then they will die a painful death. Another option is if at least one of the functions return true, then it will tell them that they are actually immortal.  
import random
import math
...
def pre_calculation(vacation, enemies):    
...
def calculation(pre_calculation):
    determine = random.randint(1,10)
    situation = (pre_calculation * determine) - abs(pre_calculation - determine)    
    counterSituation = pre_calculation * random.random()
...
def death(years, calculation, calculation2):
    if calculation and calculation2:
        return """
You will die in {} years. You will die while you are asleep. Congratulations, it will be a peaceful death.
            """.format(years)
    elif calculation and calculation2 == False:
        return """
You will die in {} years. There will be a devastating car crash that will ultimately result in your death. Sorry, it will be a painful death.
            """.format(years)
    elif calculation or calculation2:
        return """
...
#Processing
    ...
    calc = pre_calculation(location, numberOfEnemies)   
    determine = calculation(calc)
